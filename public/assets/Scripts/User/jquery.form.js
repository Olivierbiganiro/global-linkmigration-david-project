...
$(document).ready(function() {
    ...
    $("#contact").validate({
        ...
        messages: {
            ...
            email: {
                required: "Required..!",
                validate_email: "Enter valid email id"
            }
        },

        submitHandler: function(form) {
            $('#loading').addClass('loading__anim');
            $.ajax({
                url: 'contact.php',
                ...
                success: function(data) {
                    ...
                    $('#loading').removeClass('loading__anim');
                    if (data == 'error') {
                        $('.contact-msg2').html('<div class="notifications alert alert-danger alert-block"><div>Error in sending form</div><button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>');
                    } else {
                        $('.contact-msg2').html('<div class="alert alert-success d-flex align-items-center">Thank you for contact us. We received your message and will get to you soon. <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>');
                        $('#contact')[0].reset();
                    }
                }
            });
        ...
        submitHandler: function(form) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('#loading').removeClass('loading__anim');
                    if (data == 'error') {
                        $('.contact-msg').html('<div class="notifications alert alert-danger alert-block"><div>Error in sending form</div><button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>');
                    } else {
                        $('.contact-msg').html('<div class="alert alert-success d-flex align-items-center">Thank you for contact us. We received your message and will get to you soon. <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div>');
                        $('#booking')[0].reset();
                    }
                }
            });
...
$(document).ready(function() {
    $.validator.addMethod("validdate", function(t, e) {
        return this.optional(e) || 0 == Date.parseExact(t, "yyyy-M-dd")
    }, "Invalid Date"), $.validator.addMethod("indiapostalcode", function(t, e) {
        return this.optional(e) || /^\d{6}$/.test(t)
    }, "Invalid"), $.validator.addMethod("alpha", function(t, e) {
        return this.optional(e) || t == t.match(/^[a-zA-Z\s]+$/)
    }, "Invalid"), $.validator.addMethod("minname", function(t, e) {
        return this.optional(e) || /^[a-zA-Z0-9._-]{3,50}$/i.test(t)
    }, "Min 3 chars"), $.validator.addMethod("notEqual", function(t, e, i) {
        return this.optional(e) || t != i
    }, "Please specify a different (non-default) value"), $.validator.addMethod("validpassword", function(t, e) {
        return this.optional(e) || /^.*(?=.{8,})(?=.*[\d\W]).*$/i.test(t)
    }, "Contains Symbol or digit"), $.validator.addMethod("phone", function(t, e) {
        return t = t.replace(/\s+/g, ""), this.optional(e) || t.length > 9 && t.match(/^(1-?)?(\([1-9]\d{2}\)|[0-9]\d{2})-?[0-9]\d{2}-?\d{4}$/)
    }, "xxx-xxx-xxxx format"), $.validator.addMethod("ziprange", function(t, e) {
        return this.optional(e) || /^90[2-5]\d\{2}-\d{4}$/.test(t)
    }, "Your ZIP-code must be in the range 902xx-xxxx to 905-xx-xxxx"), $.validator.addMethod("integer", function(t, e) {
        return this.optional(e) || /^-?\d+$/.test(t)
    }, "A positive or negative non-decimal number please"), $.validator.addMethod("positivenumber", function(t, e) {
        return this.optional(e) || /^[0-9]+$/.test(t)
    }, "Enter Positive No."), jQuery.validator.addMethod("postalcode", function(t, e) {
        return this.optional(e) || t.match(/(^\d{5}(-\d{4})?$)|(^\d{6}(-\d{4})?$)|(^[ABCEGHJKLMNPRSTVXYabceghjklmnpstvxy]{1}\d{1}[A-Za-z]{1} ?\d{1}[A-Za-z]{1}\d{1})$/)
    }, "Invalid zip code")
}), ! function(t) {
        ...
        prototype: {
            ...
            check: function(e) {
                ...
                for (s in r) {
                    ...
                    try {
                        if (i = t.validator.methods[s].call(this, l, e, n.parameters), "dependency-mismatch" === i && 1 === a) {
                            o = !0;
                            continue
                        }
